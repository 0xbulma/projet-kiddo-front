...
export default function CommentSection({
  ...
  const [getActiveUser, { loading, error, data: activeUser }] =
    useLazyQuery(GET_BY_EMAIL);
  ...
  return (
    <div>
      <section className="container mx-auto px-10 max-h-[50rem] overflow-y-auto">
        ...
        {error !== undefined && (
          <p>Erreur lors du chargement de l'utilisateur</p>
        )}
      </section>
    </div>
  );
...
function Comment({ user, comment, refetchComments, commentTarget, targetID }) {
  ...
  const [removeComment, { called, data }] = useMutation(
    CommentsMutation.REMOVE_COMMENT
  );
  ...
  return (
    <section className="flex flex-col mt-5">
      <article className="flex items-center mt-5">
        ...
        <div className="shrink-0 z-10">
          <Link to={'../user/' + comment.sender._id}>
            <img
              src={
                comment.sender.profil_picture !== undefined &&
                comment.sender.profil_picture.thumbnail !== null
        ...
        <article className="bg-gray-300 grow rounded-lg flex flex-col ">
          ...
          <p className="p-3 break-all overflow-hidden max-h-52">
      ...
      {comment.child != null && comment.child.length > 0 && !hiddingResponse && (
        <p
          className="ml-24 mt-2 cursor-pointer hover:text-fuchsia-600 transition-all"
          onClick={toggleHiddingResponse}
        >
          <span className="mr-2">▲</span>
          Masquer{' '}
          {comment.child.length > 1
            ? 'les ' + comment.child.length + ' réponses'
            : 'la réponse'}
        </p>
      )}
      ...
      {comment.child != null && comment.child.length > 0 && hiddingResponse && (
        <p
          className="ml-24 mt-2 cursor-pointer hover:text-fuchsia-400 transition-all"
          onClick={toggleHiddingResponse}
        >
          <span className="mr-2">▼</span>
          Afficher{' '}
...
function WriteComment({
  ...
  const [createComment, { called, data }] = useMutation(
    CommentsMutation.CREATE_COMMENT
  );
      ...
      <article className="flex items-center mt-5">
        {parentId !== undefined && parentId !== null ? (
          <div className="shrink-0 flex items-center">
            ...
            <img
              src={
                user.profil_picture !== undefined &&
                user.profil_picture.thumbnail !== undefined
...
function canManageComment(activeUser, comment) {
  if (activeUser._id !== undefined && comment.sender._id !== undefined) {
    if (activeUser._id === comment.sender._id) return true;
      ...
      return (
        activeUser.rank === 'MODERATOR' || activeUser.rank === 'ADMINISTRATOR'
      );
  }
